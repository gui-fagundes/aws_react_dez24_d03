...
const ProductDetail = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      ...
      <div className="w-273 max-w-full flex flex-col items-center  md:flex-row justify-center max-h-136 my-40 md:my-10">
        <div className="bg-bl-100 flex justify-center items-center max-w-[50%] relative">
          <img
            src="/src/icons/Chevron Right.png"
            alt="NextImage"
            className="absolute top-[50%] right-0 cursor-pointer h-7 w-6"
            onClick={() => {
              if (displayImage == product.imagesUrl.length - 1) return;
              setDisplayImage(displayImage + 1);
            }}
          />
          <img src={product.imagesUrl[displayImage]} alt="" className="" />

          <img
            src="/src/icons/Chevron Left.png"
            alt="PreviousImage"
            className="absolute top-[50%] left-0 cursor-pointer h-7 w-6"
            onClick={() => {
        ...
        <div className="flex flex-col w-136 max-w-[50%] items-center md:items-end">
          <div className="flex flex-col items-start justify-evenly gap-3 text-start max-w-109 mr-20 py-4">
            <div className="flex justify-between">
              <div>
                ...
                <div className="flex flex-row gap-3 h-7">
                  ...
                  <div className="max-w-23 border-1 border-bl-100 text-bl-500 text-l1 font-medium font-inter flex items-center px-2 rounded-2xl">
            ...
            <h1 className="font-inter text-l1 font-medium leading-loose text-bl-500">
              Avaiable Colors
            </h1>
              ...
              {product.sizes.map((size) => (
                <div
                  className={`rounded-sm h-10 w-10 border-1 border-bl-100 text-center content-center cursor-pointer ${
                    selectedSize === size
                      ? "bg-bl-300 border-2 border-bl-200"
                      : ""
                  }`}
                  key={size}
      ...
      <div className="flex flex-col gap-4 mt-10 items-center max-w-screen">
        <h1 className="font-inter text-h3 font-bold text-bl-900 lg:self-start px-12">
          You might also like
        </h1>
        <h2 className="font-inter text-l1 font-medium leading-loose text-bl-300 lg:self-start px-12">
          SIMILAR PRODUCTS
        </h2>
        <Cards />
      </div>
